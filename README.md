...
* Easy, not much set-up required for general testing
* Anyone can do it (to varying degrees)* Can be used to find new errors or issues not expected in automated testing
* Recording results can be inconsistent
* Is often employed during development to see if a feature is working

### Automated Tests

* Save money and time in the long-run
* Requires setup (must write the tests)
* Increased coverage
* Repeatable
* Result accuracy; removes human error
* Easy to format and understand result
* Easy to modularize
...
After planning phases, a feature would be selected for the following steps:

1. Write a test that would confirm the feature is working correctly.
2. Write the code for the feature, ensuring it passes the test.
3. Refactor the code (clean it up, remove superfluous logs, etc.)
...
* The result, or goal met, by code is considered closely **before** writing
* Tests should pass before a feature is added to production
  * Fewer bugs and errors in production
  * Changes causing an error can be much more easily identified
* Because of the two above points, quality of code is often greatly improved
* Modularity is encouraged, making code lean toward more flexibility
* Code will be more easily extensible and far more maintainable
...
```JavaScript
const assert = require('assert').chai; // Look how easy that is!
const { sayHello, throwErrorFunc } = require('../sayHello');
